...
class ProvinciaRepositorio
{
    ...
    public function mostrarUm($provincica)
    {
        $provincica=$this->em->getRepository('AppBundle:SYS\Provincia')->findOneBy(array('id'=>$provincica));
        $pais=new PaisRepositorio($this->em);
        $provincica->setIdpais($pais->mostrarUm($provincica->getIdpais()));
        return $provincica;
    }

    public function mostrarUmx($idProvincia){
        $pais = new PaisRepositorio($this->em);
