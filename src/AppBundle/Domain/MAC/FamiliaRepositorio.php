...
use AppBundle\Entity\MAC\Familia;
...
class FamiliaRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Familia')
            ->findBy([], ['nome' => 'ASC']);
        return $cores;
    }

    public function mostrarUm($familia)
    {
        return $this->em->getRepository('AppBundle:MAC\Familia')->findOneBy(array('id' => $familia));
    }

    public function criarUm($familia)
    {
    ...
    public function actualizarUm($familia)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Familia')->findOneBy(array('id' => $familia->getId()));
        $cat->setNome($familia->getNome());
        $cat->setDescricao($familia->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($familia)
    {
