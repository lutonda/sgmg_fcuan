...
class FiloRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Filo')
            ->findBy([],['nome'=>'ASC']);
        return $cores;
    }

    public function mostrarUm($filo)
    {
        return $this->em->getRepository('AppBundle:MAC\Filo')->findOneBy(array('id' => $filo));


    }

    public function criarUm($filo)
    {
        $this->em->persist($filo);
        $this->em->flush();
        return true;
    }

    public function actualizarUm($filo)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Filo')->findOneBy(array('id' => $filo->getId()));
        $cat->setNome($filo->getNome());
        $cat->setDescricao($filo->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($filo)
    {
        $this->em->remove($filo);
        $this->em->flush();
        return true;
