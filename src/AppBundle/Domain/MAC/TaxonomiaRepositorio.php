...
class TaxonomiaRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Taxonomia')
            ->findBy([],['nome'=>'ASC']);
        return $cores;
    }

    public function mostrarUm($taxonomia)
    {
        return $this->em->getRepository('AppBundle:MAC\Taxonomia')->findOneBy(array('id' => $taxonomia));
    }

    public function criarUm($taxonomia)
    {
        $this->em->persist($taxonomia);
        $this->em->flush();
        return true;
    }

    public function actualizarUm(Taxonomia $taxonomia)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Taxonomia')->findOneBy(array('id' => $taxonomia->getId()));
        $cat->setNome($taxonomia->getNome());
        $cat->setDescricao($taxonomia->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($taxonomia)
    {
        $this->em->remove($taxonomia);
        $this->em->flush();
        return true;
    }
