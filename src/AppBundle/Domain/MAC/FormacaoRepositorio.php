...
use AppBundle\Entity\MAC\Formacao;
...
class FormacaoRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Formacao')
            ->findBy([],['nome'=>'ASC']);
        return $cores;
    }

    public function mostrarUm($formacao)
    {
        return $this->em->getRepository('AppBundle:MAC\Formacao')->findOneBy(array('id' => $formacao));
    }

    public function mostrarMineiros($mineiro)
    {
        $qb = $this->em->getRepository('AppBundle:MAC\Formacaomineiro')->findBy(array('idmineiro' => $mineiro));
        $formacaos = null;
        foreach ($qb as $formacao)
            $formacaos[] = $this->mostrarUm($formacao->getIdbrilho());;

        return $formacaos;
    }

    public function criarUm($formacao)
    {
    ...
    public function actualizarUm($formacao)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Formacao')->findOneBy(array('id' => $formacao->getId()));
        $cat->setNome($formacao->getNome());
        $cat->setDescricao($formacao->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($formacao)
    {
        $this->em->remove($formacao);
        $this->em->flush();
        return true;
    }
