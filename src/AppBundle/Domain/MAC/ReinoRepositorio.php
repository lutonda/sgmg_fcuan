...
use AppBundle\Entity\MAC\Reino;
...
class ReinoRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Reino')
            ->findBy([],['nome'=>'ASC']);
        return $cores;
    }

    public function mostrarUm($reino)
    {
        return $this->em->getRepository('AppBundle:MAC\Reino')->findOneBy(array('id' => $reino));


    }

    public function criarUm($reino)
    {
        $this->em->persist($reino);
        $this->em->flush();
        return true;
    }

    public function actualizarUm($reino)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Reino')->findOneBy(array('id' => $reino->getId()));
        $cat->setNome($reino->getNome());
        $cat->setDescricao($reino->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($reino)
    {
        $this->em->remove($reino);
        $this->em->flush();
        return true;
    }
