...
use AppBundle\Entity\MAC\Ordem;
...
class OrdemRepositorio extends EntityRepository implements UserLoaderInterface
{
    protected $em;
    ...
    public function mostrarTodos()
    {
        $cores = $this
            ->em
            ->getRepository('AppBundle:MAC\Ordem')
            ->findBy([],['nome'=>'ASC']);
        return $cores;
    }

    public function mostrarUm($ordem)
    {
        return $this->em->getRepository('AppBundle:MAC\Ordem')->findOneBy(array('id' => $ordem));


    }

    public function criarUm($ordem)
    {
    ...
    public function actualizarUm($ordem)
    {

        $cat = $this->em->getRepository('AppBundle:MAC\Ordem')->findOneBy(array('id' => $ordem->getId()));
        $cat->setNome($ordem->getNome());
        $cat->setDescricao($ordem->getDescricao());
        $this->em->flush();
        return true;
    }

    public function eliminarUm($ordem)
    {
